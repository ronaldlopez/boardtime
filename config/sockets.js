...
module.exports.sockets = {
  ...
  ***************************************************************************/
  onConnect: function(session, socket) {
    ...
    socket.join('boardtime');

    socket.on('boardtime', function(msg){
      console.log("User connect boardtime room");
    });

    socket.on('server', function(msg){
      console.log("User Peer to peer");
      socket.broadcast.to('boardtime').emit('server', { data : msg.data });
    });

    var size = 0;

    socket.on('paint', function(msg){
      size += Buffer.byteLength(msg.data, 'utf8');
      console.log(size + " bytes");
      socket.broadcast.to('boardtime').emit('paint', { data : msg.data });
    });
